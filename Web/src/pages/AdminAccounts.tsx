...
import { BookOpen, CheckCircle2, Circle, Copy, Loader2, Plus, RefreshCw, Save, Search, ShieldCheck, UserPlus } from 'lucide-react';
import toast from 'react-hot-toast';
import AdminModal from '../components/admin/AdminModal';
import AdminShell from '../components/admin/AdminShell';
import client from '../api/client';
import type { AdminAccount, AdminCreateAccountResponse, AdminManagedCourse, ApiResponse } from '../types';
...
const AdminAccounts = () => {
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      toast.success(created.sync_message ? '账号已添加，课程同步失败，可稍后手动同步' : `账号已添加，同步 ${created.sync_count} 门课程`);
    } catch (error: unknown) {
  ...
  return (
    ...
    >
        ...
        <section className="bg-white rounded-[1.75rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-100 shrink-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-400">勾选的课程会出现在首页代签同学列表中。</p>
              </div>
              <button
          ...
          <div className="p-4">
            <div className="flex items-center justify-between px-1 text-xs mb-3">
              ...
              {courses.length > 0 && (
                <button
                  onClick={() => setSelectedCourseKeys(selectedCourseKeys.length === courses.length ? [] : courses.map(courseKey))}
      ...
      <AnimatePresence>
        {showAddAccount && (
          <AdminModal title="添加学习通账号" onClose={() => setShowAddAccount(false)}>
        ...
        {showAddCourse && (
          <AdminModal title="手动添加课程" onClose={() => setShowAddCourse(false)}>
            <div className="space-y-3">
        ...
        {showCopyPanel && (
          <AdminModal title="一键套用课程" onClose={() => setShowCopyPanel(false)}>
            <div className="space-y-3">
